...
当满足 **电脑运行时间 + 用户空闲时间** 条件后，根据 **联网状态与策略**，自动通过 **pushplus 微信渠道 / App 渠道** 发送通知，并可 **进入可取消的休眠或锁屏流程**。
...
当检测到联网且消息发送成功时，支持按“提醒次数后执行超时动作”配置：

| 配置值 | 行为                                   |
| ------ | -------------------------------------- |
| 0      | 仅提醒：发送 pushplus 通知，不执行超时动作       |
| 1      | 提醒后执行动作：发送通知后弹出休眠/锁屏倒计时   |
| N>1    | 提醒 N 次后执行动作：第 N 次触发进入休眠或锁屏流程 |

---

### 5️⃣ 离线 / 发送失败：可取消超时动作倒计时
...
- 开启后在关键节点弹出系统气泡，如联网提醒成功/失败、进入可取消休眠/锁屏提示等
...
```json
{
  ...
  "remind_template_hibernate": "{base_info}\n\n建议：确认是否需要关机或休眠。",
  "remind_template_lock": "{base_info}\n\n建议：确认是否需要先锁屏。",
